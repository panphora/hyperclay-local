...
if ($tenantId) {
    Write-Host "   [OK] AZURE_TENANT_ID: $($tenantId.Substring(0,8))..." -ForegroundColor Green
} else {
    Write-Host "   [FAIL] AZURE_TENANT_ID: Not set" -ForegroundColor Red
}

if ($clientId) {
    Write-Host "   [OK] AZURE_CLIENT_ID: $($clientId.Substring(0,8))..." -ForegroundColor Green
} else {
    Write-Host "   [FAIL] AZURE_CLIENT_ID: Not set" -ForegroundColor Red
}

if ($clientSecret) {
    Write-Host "   [OK] AZURE_CLIENT_SECRET: ***SET***" -ForegroundColor Green
} else {
    Write-Host "   [FAIL] AZURE_CLIENT_SECRET: Not set" -ForegroundColor Red
}
...
if (Test-Path $metadataPath) {
    Write-Host "   [OK] Found at: $metadataPath" -ForegroundColor Green
    ...
    Write-Host "   [FAIL] Not found at expected path" -ForegroundColor Red
}
...
if ($module) {
    Write-Host "   [OK] Version: $($module.Version)" -ForegroundColor Green
} else {
...
if (Test-Path $installerPath) {
    ...
    Write-Host "   [OK] Found: $installerPath" -ForegroundColor Green
    Write-Host "   Size: $([math]::Round($fileInfo.Length / 1MB, 2)) MB" -ForegroundColor Cyan
} else {
    Write-Host "   [FAIL] Not found: $installerPath" -ForegroundColor Red
...
try {
    ...
    Write-Host "   [OK] Endpoint reachable (Status: $($response.StatusCode))" -ForegroundColor Green
} catch {
    Write-Host "   [FAIL] Cannot reach endpoint: $_" -ForegroundColor Red
}

Write-Host ""
Write-Host "=== Debug Complete ===" -ForegroundColor Cyan
Write-Host ""
Write-Host "If all checks pass, try signing manually with this command:" -ForegroundColor Yellow
Write-Host ""
Write-Host "Invoke-TrustedSigning -Endpoint https://eus.codesigning.azure.net -CodeSigningAccountName Hyperclay -CertificateProfileName HyperclayLocalPublicCertProfile -Files ""dist\HyperclayLocal-Setup-1.1.0.exe""" -ForegroundColor Cyan
Write-Host ""
